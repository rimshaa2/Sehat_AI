...
def process_ai_logic(user_text: str, user_profile: dict, doctors: list, language: str = "en-US") -> dict:
    ...
    Returns a dict:
      {
        "text": str,               # main AI response
        "suggested_doctors": list  # list of doctor dicts to show as cards
      }
    """
    if not groq_client:
        return {
    ...
    try:
        ...
        # ── Extract doctor IDs for suggestion parsing ─────────────────────────
...
10. LANGUAGE: {lang_note}"""
        ...
        # ── Parse SUGGEST_DOCTORS from response ───────────────────────────────
        suggested_doctors = []
        clean_text        = raw_text
...
def text_chat():
    try:
        ...
        doctors      = data.get("doctors", [])  # NEW — list of doctor objects
...
def health():
    return jsonify({
        ...
        "ffmpeg":     "found" if os.path.exists(AudioSegment.converter or "") else "NOT FOUND",
