...
// ── Intro Messages (proper Unicode strings) ───────────────────────────────────
...
const DoctorSuggestionCard = ({
  ...
  <View style={doctorCardStyles.card}>
    ...
    <View style={doctorCardStyles.info}>
      ...
      <View style={doctorCardStyles.metaRow}>
        {doctor.experienceYears ? (
          <Text style={doctorCardStyles.meta}>
    ...
    <TouchableOpacity
      style={doctorCardStyles.bookBtn}
...
const AICallModal = ({
  ...
  onCallEnd: (transcript: Message[]) => void;
}) => {
  const [callPhase, setCallPhase] = useState<CallPhase>("idle");
  const [callDuration, setCallDuration] = useState(0);
  const [isMuted, setIsMuted] = useState(false);
  const [statusText, setStatusText] = useState(
    "Calling AI Health Assistant..."
  );
  ...
  useEffect(() => {
    return () => {
      ...
      if (callSound) callSound.unloadAsync();
    };
  }, [callSound]);
  ...
  const resetCall = () => {
    ...
    setCallPhase("idle");
    setCallDuration(0);
    setStatusText("Calling AI Health Assistant...");
    setIsMuted(false);
  };
  ...
  const speakAIGreeting = async () => {
    ...
    const greetText = selectedLanguage.includes("ur")
      ...
      : `Hello ${userName}! I am your Sehat AI Health Assistant. How are you feeling today? Tell me your symptoms and I will help you right away.`;
        ...
        const { sound } = await Audio.Sound.createAsync(
          ...
          (status) => {
            if (status.isLoaded && status.didJustFinish) {
              setCallPhase("connected");
              setStatusText("Tap 'Speak' to talk — I'm listening");
            }
          }
        );
        setCallSound(sound);
  ...
  const startCallRecording = async () => {
    ...
    try {
      ...
      setStatusText("Listening... tap Stop when done");
    } catch (err) {
  ...
  const stopCallRecording = async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        setCallPhase("connected");
        setStatusText("Couldn't hear that — tap Speak to try again");
      }
    } catch (err: any) {
      console.error("Call voice error:", err);
      setCallPhase("connected");
      setStatusText(
        err?.message?.includes("Network")
          ? "⚠️ Server unreachable — check Python server"
          : "Error — tap Speak to try again"
      );
    }
  };
      ...
      <SafeAreaView style={callStyles.container}>
        ...
        <View style={callStyles.avatarSection}>
          <View style={{ alignItems: "center", justifyContent: "center" }}>
            <PulseCircle
              active={
                callPhase === "ai_speaking" || callPhase === "ringing"
              }
              color={getPhaseColor()}
            />
            <View
              style={[
                callStyles.avatarCircle,
          ...
          <View style={callStyles.transcriptBox}>
            {callTranscript.slice(-2).map((msg) => (
              <Text
                key={msg.id}
                style={[
                  callStyles.transcriptText,
                  msg.isUser && callStyles.transcriptUser,
                ]}
                numberOfLines={2}
              >
                {msg.isUser ? "You: " : "AI: "}
        ...
        <View style={callStyles.controlsWrapper}>
          ...
          <View style={callStyles.sideControlWrap}>
            <TouchableOpacity
              style={[
                callStyles.sideControlBtn,
                isMuted && callStyles.sideControlBtnActive,
              ]}
            ...
            <Text style={callStyles.controlLabel}>
              {isMuted ? "Unmute" : "Mute"}
            </Text>
          </View>
          ...
          <View style={callStyles.sideControlWrap}>
            <TouchableOpacity
              style={[
                ...
                (!isCallActive ||
                  callPhase === "ai_speaking" ||
                  callPhase === "processing") &&
                  callStyles.sideControlBtnDisabled,
              ]}
              onPress={
                callPhase === "user_speaking"
                  ? stopCallRecording
            ...
            <Text style={callStyles.controlLabel}>
              {callPhase === "user_speaking" ? "Stop" : "Speak"}
            </Text>
          </View>
        </View>
        ...
        <Text style={callStyles.hint}>
          {callPhase === "connected"
            ? "Tap 'Speak', talk, then tap 'Stop'"
            : callPhase === "ringing"
            ? "Connecting to AI Health Assistant..."
            : ""}
        </Text>
      </SafeAreaView>
    </Modal>
...
const FilePreview = ({
  ...
  <View style={fileStyles.container}>
    ...
    <TouchableOpacity onPress={onRemove} style={fileStyles.removeBtn}>
...
const AttachMenu = ({
  ...
  visible: boolean;
  onClose: () => void;
  onFilePicked: (file: AttachedFile) => void;
}) => {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets?.[0]) {
        ...
        const name =
          asset.fileName ||
          `image_${Date.now()}.${asset.type === "video" ? "mp4" : "jpg"}`;
        onFilePicked({
          name,
          uri: asset.uri,
    ...
    <View style={attachStyles.overlay}>
      ...
      <View style={attachStyles.menu}>
        ...
        <TouchableOpacity style={attachStyles.menuItem} onPress={pickDocument}>
          <View style={[attachStyles.menuIcon, { backgroundColor: "#EEF2FF" }]}>
            <FileText size={20} color="#6366F1" />
          </View>
          <View>
            <Text style={attachStyles.menuLabel}>Document</Text>
            <Text style={attachStyles.menuSub}>PDF, Word, Excel, any file</Text>
          </View>
        </TouchableOpacity>
        <TouchableOpacity style={attachStyles.menuItem} onPress={pickImage}>
          <View style={[attachStyles.menuIcon, { backgroundColor: "#FFF7ED" }]}>
            <Paperclip size={20} color="#F59E0B" />
          </View>
          <View>
            <Text style={attachStyles.menuLabel}>Photo / Video</Text>
            <Text style={attachStyles.menuSub}>From your gallery</Text>
          </View>
        </TouchableOpacity>
        <TouchableOpacity style={attachStyles.menuItem} onPress={takePhoto}>
          <View style={[attachStyles.menuIcon, { backgroundColor: "#F0FDF4" }]}>
            <Phone size={20} color="#22C55E" />
          </View>
          <View>
            <Text style={attachStyles.menuLabel}>Camera</Text>
            <Text style={attachStyles.menuSub}>Take a photo now</Text>
          </View>
        </TouchableOpacity>
      </View>
    </View>
...
export default ({ navigation, route }: any) => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        if (
          medicines.status === "fulfilled" &&
          medicines.value?.length > 0
        ) {
          recentMedicines = medicines.value
            .slice(0, 5)
            .map(
              (m: any) => `${m.name} ${m.dose}${m.unit} (${m.freq})`
            )
            .join(", ");
        }
        ...
        if (records.status === "fulfilled" && records.value?.length > 0) {
          recentRecords = records.value
            ...
            .map(
              (r: any) =>
                `[${r.record_type}] ${r.title} on ${r.record_date}`
            )
            .join(" | ");
        }
        let recentWellness: string | null = null;
        if (
          wellness.status === "fulfilled" &&
          wellness.value?.length > 0
        ) {
  ...
  const addErrorMessage = (text: string) => {
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), text, isUser: false, isError: true },
    ]);
  };
  ...
  const handleSend = async () => {
    ...
    const queryText =
      textToSend ||
      (fileToSend
        ? `I have attached a file: ${fileToSend.name}. Please help me with this.`
        : "");
    ...
    try {
      ...
      if (permission.status !== "granted") {
        Alert.alert(
          ...
          "Please allow microphone access in your phone settings to use voice chat."
        );
        return;
      }
      await Audio.setAudioModeAsync({
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        addErrorMessage(
          "I couldn't understand the audio. Please try speaking again more clearly."
        );
      }
    } catch (error: any) {
      addErrorMessage(
  ...
  const handleCallAI = () => {
    if (chatState !== "active_chat") {
      Alert.alert(
        ...
        "Please select your preferred language before starting a call."
      );
      return;
    }
  ...
  const handleCallEnd = (transcript: Message[]) => {
    if (transcript.length > 0) {
      setMessages((prev) => [
        ...
        {
          id: Date.now(),
          text: `📞 AI Call ended (${transcript.length} exchanges). Transcript below:`,
          isUser: false,
        },
        ...transcript,
      ]);
      ...
      if (userMsg && aiMsg)
        saveToHistory(userMsg.text, aiMsg.text, "Call");
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <AttachMenu
        ...
        onFilePicked={(file) => setPendingFile(file)}
      />
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerIcons}>
          <TouchableOpacity
            ...
            style={headerCallStyles.callBtn}
            activeOpacity={0.85}
          >
            <View style={headerCallStyles.callIconCircle}>
      ...
      >
        ...
        >
          ...
          <View style={styles.largeBotContainer}>
            ...
            <TouchableOpacity
              style={headerCallStyles.avatarCallBtn}
              onPress={handleCallAI}
            >
              <View style={headerCallStyles.avatarCallIconCircle}>
                <Phone size={14} color="#FFF" />
              </View>
              <Text style={headerCallStyles.avatarCallBtnText}>
                Tap to call AI
              </Text>
            </TouchableOpacity>
          </View>
            ...
            <View style={styles.typingContainer}>
              {voiceProcessing ? (
                ...
                <>
                  ...
                  />
                  <View
                    style={[styles.dot, { backgroundColor: "#6B7280" }]}
                  />
                  <View
                    style={[styles.dot, { backgroundColor: "#374151" }]}
                  />
                </>
              )}
            </View>
        ...
        {chatState === "language_selection" && (
          <View style={styles.footer}>
            ...
            {/* ✅ FIXED: Proper Urdu text, no unicode escapes */}
            <TouchableOpacity
              style={styles.languageButton}
        ...
        {chatState === "active_chat" && (
          <View>
            ...
            <View style={styles.inputContainer}>
              ...
              {/* Attach button (replaces + button) */}
              {inputText.length === 0 && !pendingFile ? (
                <TouchableOpacity
                  style={[styles.inputIcon, attachMenuVisible && { backgroundColor: "#E0F2F1", borderRadius: 20 }]}
