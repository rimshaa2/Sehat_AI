...
export const MyAppointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      setStatsData({
        today: data.stats?.todayAppts || 0,
        confirmed: data.appointments?.filter((a: any) => a.status === "completed").length || 0, // Mock fallback
        pending: data.appointments?.filter((a: any) => a.status === "scheduled").length || 0,
        teleconsults: 0,
      });
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-[#199A8E] text-white rounded-2xl text-sm font-bold hover:bg-[#15857a] shadow-lg shadow-emerald-100 transition-all active:scale-95"
          >
            <Plus size={18} />
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center flex-wrap gap-2 mt-4 pt-4 border-t border-slate-100">
                          {apt.status === "scheduled" && (
                            <button className="inline-flex items-center gap-2 px-4 py-2 bg-[#199A8E] text-white rounded-xl text-xs font-semibold hover:bg-[#15857a] shadow-sm shadow-[#199A8E]/20 transition-all duration-200 hover:shadow-md hover:shadow-[#199A8E]/25">
                              <Check size={14} />
                              Complete
                            </button>
                          )}
                          {apt.status === "scheduled" && (
                            <button className="inline-flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 border border-red-200 rounded-xl text-xs font-semibold hover:bg-red-100 transition-all duration-200">
                              <X size={14} />
                              Cancel
                            </button>
                          )}
